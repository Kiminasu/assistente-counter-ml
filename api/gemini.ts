...
import { Hero, HeroDetails, DraftAnalysisResult, AnalysisResult, HeroStrategy, LANES, ROLES, AITacticalCounter, HeroStrategicAnalysis, AILaneRecommendation, SpellSuggestion } from "./types.js";
import { GAME_ITEMS } from './data/items.js';
import { SPELL_ICONS } from './constants.js';
...
async function handle1v1Analysis(payload: any) {
    ...
    let banInstructions = '';
    if (yourHeroDetails) {
        banInstructions = `**Parte 3: Sugestões de Banimento (banSuggestions)**\nCom base nas fraquezas do meu herói (${yourHeroDetails.name}) contra o inimigo (${enemyHeroDetails.name}), sugira EXATAMENTE 5 bans táticos para protegê-lo. A razão ('reason') deve explicar interações de habilidades específicas e o counterType deve ser 'HARD' ou 'SOFT'.`;
    } else {
        banInstructions = `**Parte 3: Sugestões de Banimento (banSuggestions)**\nCom base nas forças do herói inimigo (${enemyHeroDetails.name}), sugira EXATAMENTE 5 bans táticos para counterá-lo. A razão ('reason') deve explicar interações de habilidades específicas e o counterType deve ser 'HARD' ou 'SOFT'.`;
    }
