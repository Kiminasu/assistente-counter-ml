...
const DirectMatchupPanel: React.FC<DirectMatchupPanelProps> = ({ isLoading, data, error }) => {
        ...
        if (isLoading) {
            return (
                <div className="flex flex-col items-center justify-center h-full">
                    ...
                    <p className="mt-3 text-sm text-gray-300">A analisar confronto...</p>
                </div>
            );
        }
        ...
        if (error) {
            return (
                <div className="text-center text-red-400 p-4">
                    <p className="font-semibold">Erro ao analisar</p>
        ...
        if (!data) {
             return (
                <div className="text-center p-8 text-gray-400 flex flex-col items-center justify-center h-full">
                    ...
                    <p className="text-xs text-gray-500 mt-1">Selecione o seu herói para ver a análise.</p>
                </div>
            );
        }

        const { yourHero, enemyHero, classification, winRate, analysis } = data;
        ...
        return (
            <div className="p-4 flex flex-col h-full">
                ...
                <div className={`text-center mt-4 p-3 rounded-lg border-2 ${styles.border} bg-black bg-opacity-20`}>
                    ...
                    <p className="font-mono font-semibold text-base sm:text-lg">{winRateText}</p>
                    <p className="text-xs text-gray-400">de taxa de vitória</p>
                </div>
    ...
    return (
        <aside className="col-span-1 glassmorphism rounded-xl animated-entry flex flex-col lg:h-[85vh]">
            <h2 className="text-xl sm:text-2xl font-black text-center mb-2 tracking-wider flex-shrink-0">CONFRONTO DIRETO</h2>
